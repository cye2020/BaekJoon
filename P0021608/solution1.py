...
class Student:
    ...
    def printInfo(self):
        friends = [str(friend.num) for friend in self.friends]
        hooks = [str(hook.num) for hook in self.hooks]
        print(f'{self.num}: {", ".join(friends)} | {", ".join(hooks)}')
...
class Seat:
    ...
    def setFriend(self, student):
        self.friend = self.friends[student]
    
    # def printMine(self):
    #     mine = [self.friend, self.near, *self.loc]
    #     print(mine)

    def __lt__(self, other):
        mine = [self.friend, self.near, -self.loc[0], -self.loc[1]]
        others = [other.friend, other.near, -other.loc[0], -other.loc[1]]
        return mine < others
...
satisfy = 0
for student in students:
    print(student.num, student.satisfy)
